...
tm_shape(pburg_data_wgs)+
  tm_polygons("pct_black", palette="BuPu")+
  tm_layout(legend.outside=TRUE)

#dissatisfying!
heat_by_tract<-pburg_data_wgs %>%
  mutate(GEOID=GEOID,
            mean_temp=raster::extract(heat_data_wgs, ., fun=mean, na.rm=T),
            med_temp=raster::extract(heat_data_wgs, ., fun=median, na.rm=T))

tm_shape(heat_by_tract)+
  tm_polygons("mean_temp", palette="BuPu")+
  tm_layout(legend.outside=TRUE)
...
ggplot(pburg_heat, aes(x=heat_mean, y=pct_white))+
  geom_point()
